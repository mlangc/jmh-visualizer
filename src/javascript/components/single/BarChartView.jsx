...
export default class BarChartView extends React.Component {
  ...
  render() {
    ...
    const maxMethodNameLength = dataSet.data
      .map((element) => element.name.length)
      .reduce((previous, current) => Math.max(previous, current), 32);
    ...
    const bars = dataSet.barGroups.map((barGroup, i) => {
      return (
        ...
        >
          ...
          <ErrorBar dataKey={`${barGroup}ErrorBarInterval`} width={4} strokeWidth={2} stroke={lightBlack} />
        </Bar>
      );
    });
    ...
    return (
      <div>
        ...
        {paramNames.length > 0 && (
          <div>
            <div>
              <b>Parameter Names:</b> {paramNames.join(':')}
            </div>
            <br />
          </div>
        )}
